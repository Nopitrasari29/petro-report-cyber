...
export default function ForgotPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.detail || "Gagal mengirim permintaan reset kata sandi.");
      }
      setSuccess(true);
    } catch (err: any) {
          ...
          {success ? (
            <div className="space-y-6 text-center py-6">
              <div className="flex justify-center">
                <div className="w-16 h-16 rounded-full bg-emerald-50 border-2 border-emerald-100 flex items-center justify-center shadow-sm">
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.8} stroke="currentColor" className="w-8 h-8 text-emerald-600">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M21.75 6.75v10.5a2.25 2.25 0 0 1-2.25 2.25h-15a2.25 2.25 0 0 1-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0 0 19.5 4.5h-15a2.25 2.25 0 0 0-2.25 2.25m19.5 0v.243a2.25 2.25 0 0 1-1.07 1.916l-7.5 4.615a2.25 2.25 0 0 1-2.36 0L3.32 8.91a2.25 2.25 0 0 1-1.07-1.916V6.75" />
                  </svg>
                </div>
              </div>
              <div className="space-y-2">
                <h3 className="text-2xl font-extrabold text-stone-900">{t("Email Sent!")}</h3>
                <p className="text-sm font-semibold text-stone-700">
                  {t("Password reset instructions have been sent to")}{" "}<span className="text-petro-green font-bold">({email})</span>.
                </p>
                <p className="text-sm text-stone-500 font-semibold">
                  {t("Please check your inbox or spam folder to update your password.")}
                </p>
              </div>
              <div className="pt-4 border-t border-stone-100 mt-6">
                <Link href="/login" className="text-sm text-petro-green font-bold hover:underline">
                  {t("Back to Login")}
                </Link>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="text-center mb-8">
                <h2 className="text-2xl font-extrabold text-stone-900">{t("Forgot Password?")}</h2>
                <p className="text-sm text-stone-500 font-semibold mt-2">
                  {t("Reset instructions will be sent to your email address.")}
                </p>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="space-y-1.5">
                  <label className="block text-sm font-semibold text-stone-700">{t("Email Address")}</label>
